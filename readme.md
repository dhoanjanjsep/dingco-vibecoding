...
**For Windows users (after installing bun):**
```powershell
...
**Note for Windows users:** If you get "bun is not recognized" error, restart your terminal after installing bun.
...
4. **Optional** - Serve the project website locally:
...
npm run install-and-setup
```
*This will install bun and set up the project automatically*

**Option 2: Manual Setup (if bun is already installed)**
```powershell
bun run setup:windows-npm
```
...
```powershell
# If you prefer bun-based setup
bun run setup:windows-simple
...
- If bun is not recognized after installation, restart your terminal
